...
function AdminSupport() {
  ...
  useEffect(() => {
    const raw = window.localStorage.getItem("leadlink:admin-tickets");
    if (raw) {
      try {
        setTickets(JSON.parse(raw));
      } catch {
        window.localStorage.removeItem("leadlink:admin-tickets");
      }
    }
  }, []);

  useEffect(() => {
    window.localStorage.setItem("leadlink:admin-tickets", JSON.stringify(tickets));
  }, [tickets]);
  ...
  return (
    <div className="space-y-5 max-w-[1300px]">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Stat icon={LifeBuoy} label="Tickets abertos" value={tickets.filter(t => t.status === "open").length} />
        <Stat icon={MessageSquare} label="Em atendimento" value={tickets.filter(t => t.status === "in_progress").length} accent="text-navy" />
        <Stat icon={Bug} label="Bugs ativos" value={tickets.filter(t => t.type === "bug" && t.status !== "resolved").length} accent="text-destructive" />
        <Stat icon={Lightbulb} label="Sugestões" value={tickets.filter(t => t.type === "feature").length} accent="text-gold" />
      </div>
