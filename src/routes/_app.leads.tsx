    ...
    <div className="space-y-5 max-w-[1600px] mx-auto">
        ...
        <Card className="p-4 border-amber-500/30 bg-amber-500/5">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <div className="text-sm text-muted-foreground">Seu plano Free exibe até 15 leads. Faça upgrade para visualizar todos os contatos capturados.</div>
            </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative">
          ...
          <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por nome..." className="pl-9 w-[260px]" />
        </div>
        <select value={sourceF} onChange={(e) => setSourceF(e.target.value)} className="h-9 rounded-md border border-border bg-background px-3 text-sm">
          <option value="all">Todas as origens</option>
          {["Site", "ZAP", "OLX", "Viva Real", "Indicação", "Instagram"].map((s) => <option key={s} value={s}>{s}</option>)}
        </select>
        <select value={brokerF} onChange={(e) => setBrokerF(e.target.value)} className="h-9 rounded-md border border-border bg-background px-3 text-sm">
          <option value="all">Todos os corretores</option>
          {uniqueBrokers.map((b) => <option key={b.id} value={b.id}>{b.name}</option>)}
        </select>
        ...
        <div className="ml-auto flex items-center gap-2">
          <div className="inline-flex p-1 rounded-lg bg-secondary border border-border">
            <button onClick={() => setView("kanban")} className={`px-3 h-7 rounded-md text-xs font-medium inline-flex items-center gap-1.5 ${view === "kanban" ? "bg-card shadow-sm text-foreground" : "text-muted-foreground"}`}>
              <LayoutGrid className="h-3.5 w-3.5" /> Kanban
            </button>
            <button onClick={() => setView("table")} className={`px-3 h-7 rounded-md text-xs font-medium inline-flex items-center gap-1.5 ${view === "table" ? "bg-card shadow-sm text-foreground" : "text-muted-foreground"}`}>
              <List className="h-3.5 w-3.5" /> Tabela
            </button>
          </div>
          {plan.isFree && (
            <Button className="bg-gold text-navy hover:bg-gold/90 font-semibold" onClick={() => setUpgradeOpen(true)}>
      ...
      {allLeads.length === 0 ? (
        <EmptyState
          ...
          action={<Button asChild><Link to="/meu-link">Compartilhar Meu Link</Link></Button>}
        />
      ) : filtered.length === 0 ? (
        <EmptyState
          ...
          action={<Button variant="outline" onClick={() => { setSearch(""); setSourceF("all"); setBrokerF("all"); }}>Limpar filtros</Button>}
        />
      ) : view === "kanban" ? (
        <KanbanView leads={filtered} onLeadClick={handleLeadClick} />
      ) : (
      ...
      <UpgradeModal
        ...
        benefits={["Leads ilimitados", "Respostas completas do quiz", "Score e classificação comercial", "Próximo passo recomendado", "Histórico organizado no CRM"]}
...
function KanbanView({ leads, onLeadClick }: { leads: LeadRow[]; onLeadClick: (lead: LeadRow) => void }) {
  return (
          ...
          <div key={status} className="w-[300px] shrink-0">
            <div className="flex items-center justify-between mb-3 px-1">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="secondary" className="font-mono text-[10px] px-1.5">{items.length}</Badge>
              </div>
            ...
            <div className="space-y-2.5">
              {items.map((l) => {
                ...
                return isLeadBlocked ? (
                  ...
                  <LeadCard key={l.id} lead={l} onClick={() => onLeadClick(l)} />
                );
              })}
              {items.length === 0 && <div className="border border-dashed border-border rounded-lg p-6 text-center text-xs text-muted-foreground">Nenhum lead</div>}
            </div>
          </div>
        );
      })}
...
function LeadCard({ lead, onClick }: { lead: LeadRow; onClick: () => void }) {
  const initials = lead.brokerInitials || lead.brokerName?.split(" ").map((n: string) => n[0]).join("").slice(0, 2) || "?";
      ...
      <Card className="p-3.5 border-border hover:shadow-lift hover:-translate-y-px transition-all border-l-2 border-l-transparent hover:border-l-gold cursor-pointer">
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <div className="flex flex-col items-end gap-1">
            <Badge variant="outline" className={`gap-1 text-[10px] font-medium px-1.5 ${classificationBadgeClass(lead.classification)}`}>
              <ClassificationIcon className="h-3 w-3" /> {formatClassification(lead.classification)}
            </Badge>
            <Badge variant="outline" className={`text-[10px] font-mono px-1.5 ${scoreColor(lead.score)}`}>{lead.score}</Badge>
          </div>
        </div>
        <div className="flex items-center gap-2 mb-2.5">
          <Badge variant="outline" className="text-[10px] font-normal">{formatIntent(lead.intentType)}</Badge>
          <span className="text-xs text-muted-foreground line-clamp-1">{lead.nextStep || lead.interest || "Próximo passo não informado"}</span>
        </div>
...
function BlockedLeadCard({ lead, onClick }: { lead: any; onClick: () => void }) {
  return (
    <Card className="p-3.5 border-border bg-secondary/40 opacity-75 cursor-pointer hover:opacity-100 transition" onClick={onClick}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-2">
      ...
      <div className="text-xs text-muted-foreground mb-2">{new Date(lead.createdAt).toLocaleDateString("pt-BR")}</div>
      <div className="flex items-center gap-2 flex-wrap mb-2">
        {lead.source && <Badge variant="outline" className="text-[10px]">{lead.source}</Badge>}
      </div>
      <div className="text-xs text-muted-foreground">Você capturou este lead, mas precisa do plano Pro para visualizar os dados.</div>
      <Button
...
function TableView({ leads, onLeadClick }: { leads: LeadRow[]; onLeadClick: (lead: LeadRow) => void }) {
  return (
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-secondary/40">
              <th className="text-left px-4 py-3 text-[11px] uppercase tracking-wider text-muted-foreground font-medium">Nome</th>
              <th className="text-left px-4 py-3 text-[11px] uppercase tracking-wider text-muted-foreground font-medium">Interesse</th>
          ...
          <tbody className="divide-y divide-border">
            {leads.map((l) =>
              Boolean(l.isBlocked) ? (
                <tr key={l.id} className="hover:bg-secondary/30 cursor-pointer opacity-60" onClick={() => onLeadClick(l)}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2"><Lock className="h-3.5 w-3.5 text-muted-foreground" /><span className="text-muted-foreground">Lead bloqueado</span></div>
                  </td>
                  <td className="px-4 py-3 text-muted-foreground">—</td>
                  <td className="px-4 py-3"><Badge variant="outline" className="text-[10px]">Pro</Badge></td>
                  <td className="px-4 py-3 text-muted-foreground">—</td>
                ...
                <tr key={l.id} className="hover:bg-secondary/30 cursor-pointer" onClick={() => onLeadClick(l)}>
                  ...
                  <td className="px-4 py-3 text-muted-foreground truncate max-w-[200px]">{l.interest}</td>
                  <td className="px-4 py-3"><Badge className={statusBadgeClass(l.status)}>{STATUS_LABEL[l.status as keyof typeof STATUS_LABEL] || l.status}</Badge></td>
...
function LeadCardMobile({ lead, onClick }: { lead: LeadRow; onClick: () => void }) {
  return (
    <button type="button" onClick={onClick} className="block w-full p-4 text-left hover:bg-secondary/40">
...
function BlockedLeadMobile({ lead, onClick }: { lead: any; onClick: () => void }) {
  return (
    <button className="block w-full text-left p-4 hover:bg-secondary/40" onClick={onClick}>
      <div className="flex items-start justify-between mb-1"><div className="flex items-center gap-2"><Lock className="h-4 w-4 text-muted-foreground" /><div className="font-medium">Lead bloqueado</div></div><Badge variant="outline" className="text-[10px]">Pro</Badge></div>
      <div className="text-xs text-muted-foreground mb-2">{new Date(lead.createdAt).toLocaleDateString("pt-BR")}</div>
      <div className="text-xs text-muted-foreground">Você capturou este lead, mas precisa do plano Pro para visualizar os dados.</div>
    </button>
  );
            ...
            <DrawerHeader className="border-b border-border px-5 pb-4 text-left">
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <DrawerTitle className="truncate text-xl">{formatLeadValue(lead.name)}</DrawerTitle>
                ...
                <Card className="p-5">
                  <div className="mb-4 flex flex-wrap items-center gap-2">
                    <Badge className={statusBadgeClass(lead.status)}>
                      {STATUS_LABEL[lead.status as keyof typeof STATUS_LABEL] || lead.status || "Status não informado"}
                    </Badge>
                    <Badge variant="outline" className={`font-mono ${scoreColor(lead.score)}`}>
                      Score {formatLeadValue(lead.score)}
                    </Badge>
                    <Badge variant="outline" className={classificationBadgeClass(lead.classification)}>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <DetailField label="Status" value={STATUS_LABEL[lead.status as keyof typeof STATUS_LABEL] || lead.status} />
                    <DetailField label="Intenção" value={formatIntent(lead.intentType)} />
                    <DetailField label="Classificação" value={formatClassification(lead.classification)} />
                    <DetailField label="Score" value={lead.score} />
                    <DetailField label="Próximo passo" value={lead.nextStep} className="sm:col-span-2" />
                    <DetailField label="Resumo do perfil" value={lead.profileSummary} className="sm:col-span-2" />
                    <DetailField label="Data de criação" value={formatDateTime(lead.createdAt)} className="sm:col-span-2" />
                  </div>
                </Card>
                  ...
                  {getQuizEntries(lead).length > 0 ? (
                    <div className="space-y-3">
                      {getQuizEntries(lead).map(([key, value]) => (
                        <DetailField key={key} label={ANSWER_LABELS[key] || humanizeAnswerKey(key)} value={formatAnswerValue(value)} />
                      ))}
                    </div>
                  ) : (
                    <div className="text-sm text-muted-foreground">Nenhuma resposta de quiz disponível.</div>
                  )}
                </Card>
              </div>
            </div>
...
function formatLeadValue(value: unknown) {
  ...
  if (Array.isArray(value)) return value.map(formatAnswerValue).filter(Boolean).join(", ") || "Não informado";
...
function formatAnswerValue(value: unknown): string {
  ...
  if (typeof value === "boolean") return value ? "Sim" : "Não";
  if (typeof value === "number") return String(value);
  if (Array.isArray(value)) return value.map(formatAnswerValue).filter((item) => item !== "Não informado").join(", ");
