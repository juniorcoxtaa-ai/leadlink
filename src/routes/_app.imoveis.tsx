...
function ImoveisPage() {
  ...
  const filtered = useMemo(() => {
    return items.filter((p) => {
      ...
      return [p.code, p.title, p.neighborhood, p.city, p.highlight].some((v) => String(v || "").toLowerCase().includes(q));
    });
  }, [items, query, activeType]);

  const total = filtered.length;
  const disp = filtered.filter((p) => p.status === "Disponível").length;
  const vgv = filtered.filter((p) => p.status !== "Vendido").reduce((s, p) => s + Number(p.price || 0), 0);
    ...
    <div className="space-y-6 max-w-[1500px] mx-auto">
        ...
        <div className="flex gap-2">
          {mySlug ? (
            <Button asChild variant="outline" className="rounded-full">
              <Link to="/l/$slug/vitrine" params={{ slug: mySlug }} target="_blank"><ExternalLink className="h-4 w-4 mr-1.5" /> Ver vitrine pública</Link>
            </Button>
          ) : (
            <Button variant="outline" className="rounded-full" onClick={() => toast.error("Configure seu Meu Link primeiro para abrir sua vitrine pública")}>
          ...
          <Button className="bg-navy text-navy-foreground hover:bg-navy/90 rounded-full" onClick={handleCreateClick}>
      ...
      <Card className="p-3 border-border/70 flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <Input placeholder="Buscar por código, bairro, característica…" value={query} onChange={(e) => setQuery(e.target.value)} className="pl-9 border-transparent bg-secondary/60 focus-visible:bg-card" />
        </div>
        {["Todos", "Apartamento", "Cobertura", "Casa", "Studio", "Comercial"].map((t) => (
          <button key={t} onClick={() => setActiveType(t)} className={`rounded-full px-3 py-1.5 text-xs border transition ${activeType === t ? "bg-navy text-navy-foreground border-navy" : "border-border bg-background hover:bg-secondary"}`}>{t}</button>
        ))}
      </Card>
      ...
      {items.length === 0 ? (
        ...
        <EmptyState
          ...
          action={<Button variant="outline" onClick={() => { setQuery(""); setActiveType("Todos"); }}>Limpar filtros</Button>}
        />
      ) : (
        ...
        {filtered.map((p) => (
          <Card key={p.id} className="overflow-hidden border-border/70 group hover:shadow-lift transition-all p-0">
            <div className="relative aspect-[4/3] overflow-hidden bg-secondary">
              {safeSrc(p.image) ? (
                <img src={safeSrc(p.image)} alt={p.title} loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              ) : (
              ...
              <div className="absolute top-3 left-3 flex gap-2">{p.highlight && <span className="px-2.5 py-1 rounded-full bg-navy/90 text-gold text-[10px] font-semibold uppercase tracking-wider backdrop-blur">{p.highlight}</span>}</div>
              <div className="absolute top-3 right-3"><Badge variant="outline" className={`${statusStyle[p.status] || ""} rounded-full font-medium backdrop-blur bg-card/80`}>{p.status}</Badge></div>
              <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between text-white">
                <div className="bg-card/95 backdrop-blur rounded-lg px-3 py-1.5"><div className="font-display text-lg font-semibold leading-none text-emerald">{fmtBRL(p.price)}</div></div>
                <div className="bg-card/95 backdrop-blur rounded-full px-2.5 py-1 text-[11px] font-mono text-foreground">{p.code}</div>
              </div>
            </div>
            <div className="p-5 space-y-3">
              <div>
                <div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground flex items-center gap-1"><MapPin className="h-3 w-3" /> {p.neighborhood} · {p.city}</div>
              ...
              <div className="flex items-center gap-4 text-xs text-muted-foreground border-y border-border py-2.5">
                {Number(p.bedrooms) > 0 && <span className="flex items-center gap-1"><Bed className="h-3.5 w-3.5" />{p.bedrooms}</span>}
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Avatar className="h-7 w-7 ring-1 ring-border"><AvatarFallback className="bg-navy text-gold text-[10px] font-semibold">{(p.brokerInitials || p.brokerName || "LL").slice(0, 2)}</AvatarFallback></Avatar>
                  <div className="text-[11px] text-muted-foreground leading-tight"><div className="font-medium text-foreground">{(p.brokerName || "Leadlink").split(" ")[0]}</div><div>Captador</div></div>
                </div>
                <div className="flex items-center gap-3 text-[11px] text-muted-foreground"><span className="flex items-center gap-1"><Eye className="h-3 w-3" />{p.views}</span><span className="flex items-center gap-1"><Users className="h-3 w-3" />{p.leadsCount ?? p.leads ?? 0}</span></div>
              </div>
              <div className="flex items-center gap-2 pt-1">
                <select value={p.status} onChange={(e) => {
                  const next = e.target.value;
                  setItems((prev) => prev.map((item) => (item.id === p.id ? { ...item, status: next } : item)));
                  updatePropertyStatus({ data: { id: p.id, status: next } }).catch((error: any) => toast.error(error?.message || "Falha ao atualizar status"));
      ...
      <UpgradeModal
        ...
        benefits={["Imóveis ilimitados", "Vitrine mais completa", "Mais opções para seus leads", "Imagem de fundo, vídeos e quiz editável no Meu Link"]}
        ...
        <div className="fixed inset-0 pointer-events-none">
          <div className="absolute right-6 bottom-6 hidden xl:block w-[320px]">
            <Card className="p-4 bg-secondary/70 border-border/70 shadow-xl">
              <div className="flex items-center gap-2 mb-3"><Badge>Pro</Badge><span className="text-sm font-medium">Preview do Pro</span></div>
