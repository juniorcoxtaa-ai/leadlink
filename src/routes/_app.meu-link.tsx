...
import {
  Copy, ExternalLink, QrCode, Save, Plus, Trash2,
  User, Palette, Link2, Sparkles, Eye,
  HelpCircle, Home, Lock, GripVertical, Check, MessageCircle,
  Upload, Image as ImageIcon, X, BadgeCheck, Type, Square, Video as VideoIcon,
} from "lucide-react";
import { toast } from "sonner";
import { getProperties } from "@/server-fns/properties";
import { getMyProfile } from "@/server-fns/profile";
import {
  EMPTY_MEU_LINK_CONFIG, uploadImage, loadConfig, saveConfig,
  ACCENT_TOKENS, BG_PRESETS, FONT_FAMILIES, BTN_RADIUS,
  type MeuLinkConfig, type Question, type QuestionType,
  type Accent, type BgStyle, type FontStyle, type BtnShape,
} from "@/lib/meu-link-store";
import { DEFAULT_QUIZ_BLOCKS, type QuizBlocks, type QuizIntent, type QuizQuestion } from "@/lib/quiz-blocks";
...
function MeuLinkPage() {
  ...
  const removeStat = (id: string) => update("stats", cfg.stats.filter((s) => s.id !== id));
  const updateStat = (id: string, field: "label" | "value", v: string) =>
    update("stats", cfg.stats.map((s) => (s.id === id ? { ...s, [field]: v } : s)));

  // LINKS
  const addLink = () =>
    update("links", [...cfg.links, { id: Date.now().toString(), label: "Novo link", url: "", enabled: true }]);
  const removeLink = (id: string) => update("links", cfg.links.filter((l) => l.id !== id));
  const updateLink = (id: string, field: "label" | "url" | "enabled", v: string | boolean) =>
    update("links", cfg.links.map((l) => (l.id === id ? { ...l, [field]: v } : l)));
  ...
  const renderQuizBlock = (intent: QuizIntent, title: string, subtitle: string) => {
    ...
    return (
      <div className={`rounded-xl border p-4 space-y-3 ${block.enabled ? "border-border bg-card" : "border-border bg-muted/30 opacity-70"}`}>
          ...
          {block.questions.map((q) => (
            <div key={q.id} className="rounded-lg border border-border p-3 space-y-2">
              <div className="flex items-center gap-2">
                <Input value={q.label} onChange={(e) => updateQuizQuestion(intent, q.id, { label: e.target.value })} className="font-medium text-sm" />
                <Switch checked={q.enabled !== false} onCheckedChange={(v) => toggleQuizQuestion(intent, q.id, v)} />
              </div>
              <div className="grid gap-2 md:grid-cols-[1fr_160px]">
                <Input
                  value={q.placeholder || ""}
                  onChange={(e) => updateQuizQuestion(intent, q.id, { placeholder: e.target.value })}
              ...
              {q.type === "select" && (
                <div className="space-y-1.5">
                  <Input
                    ...
                    onChange={(e) => updateQuizOptions(intent, q.id, e.target.value.split("|").map((s) => s.trim()).filter(Boolean))}
                    placeholder="Opção 1 | Opção 2 | Opção 3"
                    className="text-xs font-mono h-8"
                  />
                  <div className="text-[10px] text-muted-foreground">Separe as opções por <code>|</code></div>
                </div>
              )}
            </div>
          ))}
  ...
  const selectedQuizBlock = renderQuizBlock(
    activeQuizIntent,
    activeQuizIntent === "locacao" ? "Locação" : activeQuizIntent === "compra" ? "Compra" : "Investimento",
  ...
  const toggleFeatured = (id: string) => {
    if (cfg.featuredIds.includes(id)) update("featuredIds", cfg.featuredIds.filter((f) => f !== id));
    else {
  ...
  const removeVideo = (id: string) =>
    update("videos", (cfg.videos ?? []).filter((v) => v.id !== id));
  const updateVideo = (
    id: string,
    patch: Partial<{ title: string; url: string; enabled: boolean }>,
  ) => update("videos", (cfg.videos ?? []).map((v) => (v.id === id ? { ...v, ...patch } : v)));
    ...
    <div className="space-y-6 max-w-[1400px] mx-auto">
      ...
      {missingSlug && (
        <EmptyState
          ...
          action={<Button asChild><Link to="/configuracoes">Configurar slug</Link></Button>}
          className="py-7"
        />
      )}
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <div className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">LeadLink · Corretor</div>
          <h1 className="font-display text-3xl md:text-4xl font-semibold tracking-tight mt-1">Meu link</h1>
          <p className="text-sm text-muted-foreground mt-1">
            Bom te ver, {cfg.name.split(" ")[0]}. Edite ao vivo — as alterações são salvas automaticamente.
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <Card className="p-4 border-border/70 flex items-center gap-3">
          <div className="h-9 w-9 rounded-lg bg-emerald/10 text-emerald flex items-center justify-center"><Sparkles className="h-4 w-4" /></div>
          <div className="min-w-0">
            <div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Quiz</div>
            <div className="text-sm font-medium truncate">{enabledBlockCount} blocos ativos</div>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px] border-emerald/30 text-emerald">Pronto</Badge>
        </Card>
        <Card className="p-4 border-border/70 flex items-center gap-3">
          <div className="h-9 w-9 rounded-lg bg-gold/15 text-gold flex items-center justify-center"><Home className="h-4 w-4" /></div>
          <div className="min-w-0">
            <div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Imóveis em destaque</div>
            <div className="text-sm font-medium truncate">{cfg.featuredIds.length} selecionados</div>
          </div>
          <Badge variant="outline" className="ml-auto text-[10px]">máx. 6</Badge>
        </Card>
        <Card className="p-4 border-border/70 flex items-center gap-3">
          <div className="h-9 w-9 rounded-lg bg-navy/10 text-navy flex items-center justify-center"><Link2 className="h-4 w-4" /></div>
          <div className="min-w-0 flex-1">
            <div className="text-[10px] uppercase tracking-[0.16em] text-muted-foreground">Link</div>
            <div className="text-sm font-mono truncate">{profile?.slug ? fullUrl : "Configure seu perfil para gerar o link"}</div>
          </div>
        </Card>
      </div>
        ...
        <Card className="border-border/70 p-0 overflow-hidden">
          ...
          <Tabs defaultValue="profile" className="p-6">
            <TabsList className="grid grid-cols-6 w-full bg-secondary">
              ...
              <TabsTrigger value="appearance" className="gap-1.5"><Palette className="h-3.5 w-3.5" /> Visual</TabsTrigger>
              <TabsTrigger value="quiz" className="gap-1.5"><HelpCircle className="h-3.5 w-3.5" /> Quiz</TabsTrigger>
              <TabsTrigger value="videos" className="gap-1.5"><VideoIcon className="h-3.5 w-3.5" /> Vídeos</TabsTrigger>
              <TabsTrigger value="featured" className="gap-1.5"><Home className="h-3.5 w-3.5" /> Imóveis</TabsTrigger>
            ...
            <TabsContent value="profile" className="space-y-6 mt-6">
              <Section icon={<User className="h-3.5 w-3.5" />} title="Identidade">
                ...
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <div className="h-20 w-20 rounded-full overflow-hidden ring-2 ring-border bg-gold flex items-center justify-center text-navy font-bold text-xl">
                      {safeSrc(cfg.photoUrl) ? (
                        ...
                        cfg.name.split(" ").map((p: any) => p[0]).slice(0, 2).join("").toUpperCase()
                      )}
                    </div>
                    {cfg.photoUrl && (
                      <button
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[11px] text-muted-foreground">JPG ou PNG · até 4 MB</span>
                    </div>
                  </div>
                </div>
                ...
                <Field label="Especialidade (subtítulo)" hint="Aparece abaixo do nome">
                  <Input value={cfg.subtitle} onChange={(e) => update("subtitle", e.target.value)} />
                </Field>
                <Field label="Apresentação (bio)" required>
                  <Textarea rows={3} value={cfg.bio} onChange={(e) => update("bio", e.target.value)} />
                </Field>
                ...
                <div className="flex items-center justify-between rounded-lg border border-border p-3">
                  ...
                  <div className="text-xs text-muted-foreground">Exibe um badge de corretor verificado</div>
                  </div>
              ...
              <Section icon={<Link2 className="h-3.5 w-3.5" />} title="URL da página">
                <Field label="Endereço personalizado" required hint="Letras minúsculas, números e hífens">
              ...
              <Section icon={<Sparkles className="h-3.5 w-3.5" />} title="Prova social" subtitle="Até 3 estatísticas">
                <div className="space-y-2">
                  {cfg.stats.map((s) => (
                    <div key={s.id} className="grid grid-cols-[1fr_1fr_auto] gap-2">
                      <Input value={s.value} onChange={(e) => updateStat(s.id, "value", e.target.value)} placeholder="240+" />
                      <Input value={s.label} onChange={(e) => updateStat(s.id, "label", e.target.value)} placeholder="Clientes" />
                  ...
                  {cfg.stats.length < 3 && (
                    <Button variant="outline" size="sm" className="w-full rounded-lg border-dashed" onClick={addStat}>
            ...
            <TabsContent value="appearance" className="space-y-6 mt-6">
              <Section icon={<Palette className="h-3.5 w-3.5" />} title="Cor de destaque" subtitle="Acentua botões, ícones e selos">
                    ...
                    return (
                      ...
                      >
                        <div
                          ...
                          style={{ background: t.bg, boxShadow: active ? `0 0 0 3px ${t.ring}` : undefined }}
                        />
                        {t.label}
                      </button>
                    );
              ...
              <Section icon={<Sparkles className="h-3.5 w-3.5" />} title="Estilo de fundo" subtitle="Pré-visualizações ao vivo">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {(Object.keys(BG_PRESETS) as Exclude<BgStyle, "image">[]).map((id) => {
                    ...
                    return (
                      ...
                      >
                        <div className="h-12 rounded-md mb-1.5" style={{ background: p.preview }} />
                        {p.label}
                      </button>
                    );
                  })}
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(Object.keys(FONT_FAMILIES) as FontStyle[]).map((f) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-lg font-semibold leading-none mb-1" style={{ fontFamily: ff.family }}>Aa</div>
                ...
                <div className="rounded-xl border border-dashed border-border p-4 space-y-3">
                  {cfg.bgImage ? (
                    <div className="relative rounded-lg overflow-hidden h-40 group">
                      {safeSrc(cfg.bgImage) ? (
                        <img src={safeSrc(cfg.bgImage)} alt="bg" className="absolute inset-0 h-full w-full object-cover" />
                      ) : null}
                      <div
            ...
            <TabsContent value="videos" className="space-y-4 mt-6">
              <div className="rounded-xl border border-border bg-secondary/40 p-4 flex items-start gap-3">
                ...
                <div className="text-xs text-muted-foreground leading-relaxed">
                  Adicione blocos de vídeo (YouTube ou Vimeo). Cole a URL completa — o player aparece direto na sua página.
                  <span className="block mt-1 text-emerald font-medium">{(cfg.videos ?? []).length} / 6 blocos</span>
                </div>
              </div>
                  ...
                  {(cfg.videos ?? []).map((v) => (
                    <div key={v.id} className="rounded-xl border border-border p-3 space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        />
                        <Switch checked={v.enabled} onCheckedChange={(b) => updateVideo(v.id, { enabled: b })} />
            ...
            <TabsContent value="quiz" className="space-y-6 mt-6">
              <div className="rounded-xl border border-border bg-secondary/40 p-4 flex items-start gap-3">
                ...
                <div className="text-xs text-muted-foreground leading-relaxed">
                  ...
                  Nome, Cidade e Telefone são coletados em todos os quizzes e não podem ser desativados.
                </div>
              </div>
              <Field label="Texto de boas-vindas do quiz" hint="Aparece antes das perguntas">
                <Textarea rows={2} value={cfg.quizIntro} onChange={(e) => update("quizIntro", e.target.value)} />
              </Field>
              ...
              <div className="rounded-xl border border-border bg-card p-3 space-y-3">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="text-[11px] text-muted-foreground">Selecione um tipo para editar apenas aquele bloco.</div>
                  </div>
                  <div className="text-xs text-muted-foreground">
                    <span className="font-semibold text-foreground">{enabledBlockCount}</span> / 3 ativos
                  </div>
                </div>
                <div className="grid grid-cols-3 gap-2">
                  {(["locacao", "compra", "investimento"] as QuizIntent[]).map((intent) => {
                    ...
                    const block = quizBlocks[intent];
                    const label = intent === "locacao" ? "Locação" : intent === "compra" ? "Compra" : "Investimento";
                    return (
                      <button
                        key={intent}
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <span className={`text-[10px] px-2 py-0.5 rounded-full ${block.enabled ? "bg-emerald/10 text-emerald" : "bg-muted text-muted-foreground"}`}>
                ...
                <div className="flex items-center justify-between">
                  <div className="text-xs text-muted-foreground">Ative ou desative o bloco selecionado abaixo.</div>
                  <Switch checked={quizBlocks[activeQuizIntent].enabled} onCheckedChange={(v) => toggleBlock(activeQuizIntent, v)} />
                </div>
              </div>
            ...
            <TabsContent value="featured" className="space-y-4 mt-6">
              <div className="rounded-xl border border-border bg-secondary/40 p-4 flex items-start gap-3">
                ...
                <div className="text-xs text-muted-foreground leading-relaxed">
                  Selecione os imóveis exibidos no botão <strong className="text-foreground">Imóveis em destaque</strong>.{" "}
                  <span className="text-emerald font-medium">{cfg.featuredIds.length} selecionados</span> · máx. 6.
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {availableProps.length === 0 && (
                  <p className="col-span-2 text-sm text-muted-foreground text-center py-4">
                    Nenhum imóvel cadastrado ainda. Adicione imóveis em <strong>Imóveis</strong> para destacar aqui.
                  </p>
                )}
                  ...
                  return (
                    ...
                    >
                      <div className="relative h-28 overflow-hidden bg-muted">
                        {safeSrc(p.image) ? (
                          <img src={safeSrc(p.image)} alt={p.title} className="h-full w-full object-cover group-hover:scale-105 transition-transform" />
                        ) : (
                        ...
                        <Badge className="absolute bottom-2 left-2 bg-background/90 text-foreground text-[10px]">{p.code}</Badge>
                      </div>
                      <div className="p-3">
                        <div className="text-sm font-medium leading-tight line-clamp-1">{p.title}</div>
                        <div className="text-[11px] text-muted-foreground mt-0.5">{p.neighborhood}</div>
                        <div className="flex items-center justify-between mt-2">
                          <div className="font-display text-sm font-semibold">R$ {(p.price / 1000).toLocaleString("pt-BR")}k</div>
                          <div className="text-[10px] text-muted-foreground">{p.bedrooms}q · {p.area}m²</div>
                        </div>
                      </div>
                    </button>
                  );
            ...
            <TabsContent value="links" className="space-y-3 mt-6">
              {cfg.links.map((l) => (
                <div key={l.id} className="rounded-xl border border-border p-3 space-y-2">
                  <div className="flex items-center gap-2">
                    <Input value={l.label} onChange={(e) => updateLink(l.id, "label", e.target.value)} placeholder="Nome do link" className="font-medium" />
                    <Switch checked={l.enabled} onCheckedChange={(v) => updateLink(l.id, "enabled", v)} />
                    <Button size="icon" variant="ghost" onClick={() => removeLink(l.id)}>
                      <Trash2 className="h-4 w-4 text-muted-foreground" />
                    </Button>
                  </div>
                  <Input value={l.url} onChange={(e) => updateLink(l.id, "url", e.target.value)} placeholder="https://..." className="font-mono text-xs" />
                </div>
              ))}
              <Button variant="outline" className="w-full rounded-lg border-dashed" onClick={addLink}>
          ...
          <div className="mx-auto w-full max-w-[340px]">
            <div className="rounded-[2.5rem] border-[10px] border-foreground/90 bg-foreground/90 shadow-lift overflow-hidden">
              <div className="rounded-[2rem] overflow-hidden bg-card">
                ...
                <MeuLinkPreview
                  cfg={cfg}
                  featuredProperties={availableProps.filter((p: any) => cfg.featuredIds.includes(p.id))}
                />
              </div>
            </div>
            <div className="text-center text-[10px] text-muted-foreground mt-2">
              Atualiza conforme você edita · clique em <span className="font-semibold">Abrir</span> para ver em tela cheia
            </div>
          </div>
        </div>
      </div>
...
function Section({
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        <div className="h-6 w-6 rounded-md bg-secondary flex items-center justify-center text-foreground/70">{icon}</div>
        <div>
...
function SaveBadge({ state }: { state: "idle" | "saving" | "saved" | "error" }) {
  ...
  const map = {
    ...
    error: { text: "Erro ao salvar", cls: "bg-destructive/10 text-destructive border border-destructive/30" },
  } as const;
  const { text, cls } = map[state];
  return (
    <span className={`text-[11px] font-medium px-2.5 py-1 rounded-full ${cls}`}>{text}</span>
  );
}
