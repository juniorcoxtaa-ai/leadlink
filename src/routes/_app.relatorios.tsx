...
const PALETTE = ["var(--color-navy)", "var(--color-gold)", "var(--color-chart-3)", "var(--color-success)", "var(--color-warning)", "var(--color-destructive)"];
    ...
    <div className="space-y-5 max-w-[1500px] mx-auto">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="ml-auto flex gap-2">
          ...
          <Button size="sm" className="bg-navy text-navy-foreground hover:bg-navy/90"><Download className="h-4 w-4 mr-1" /> PDF</Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            ...
            {isClient ? (
              <ResponsiveContainer>
                <LineChart data={data?.leadsOverTime ?? []}>
                  ...
                  <Line type="monotone" dataKey="leads" stroke="var(--color-navy)" strokeWidth={2.5} dot={false} />
                  <Line type="monotone" dataKey="ganhos" stroke="var(--color-gold)" strokeWidth={2.5} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            ) : (
        ...
        <Card className="p-5">
          ...
          <div className="h-[280px]">
            {isClient ? (
              <ResponsiveContainer>
                <PieChart>
                  <Pie data={data?.leadsBySource ?? []} dataKey="value" nameKey="name" innerRadius={50} outerRadius={90} paddingAngle={2}>
                    {(data?.leadsBySource ?? []).map((_: any, i: any) => <Cell key={i} fill={PALETTE[i % PALETTE.length]} />)}
                  </Pie>
                  <Tooltip contentStyle={{ borderRadius: 8, border: "1px solid var(--color-border)" }} />
        ...
        <Card className="p-5 lg:col-span-2">
          ...
          <div className="h-[260px]">
            {isClient ? (
              <ResponsiveContainer>
                <BarChart data={(data?.brokerStats ?? []).map((b: any) => ({ name: b.name.split(" ")[0], conv: b.conversion }))}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--color-border)" />
        ...
        <Card className="p-5 flex flex-col">
          ...
          <div className="flex-1 relative">
            {isClient ? (
              <ResponsiveContainer>
                <RadialBarChart innerRadius="65%" outerRadius="95%" data={responseGauge} startAngle={210} endAngle={-30}>
      ...
      <Card className="overflow-hidden">
        ...
        <div className="divide-y divide-border">
          {[...(data?.brokerStats ?? [])].sort((a, b) => b.conversion - a.conversion).map((b, i) => (
            <div key={b.id} className="flex items-center gap-4 p-4">
              <div className="font-mono text-xs w-6 text-muted-foreground">#{i + 1}</div>
              <Avatar className="h-9 w-9">
                <AvatarFallback className="bg-navy text-navy-foreground text-xs font-semibold">{b.initials}</AvatarFallback>
              </Avatar>
              <div className="flex-1 min-w-0">
                <div className="font-medium">{b.name}</div>
              </div>
              <div className="hidden sm:block text-sm">
                <span className="text-muted-foreground">Leads:</span> <span className="font-semibold">{b.leads}</span>
              </div>
              <Badge className="bg-gold/15 text-gold border border-gold/30 font-mono">{b.conversion}%</Badge>
            </div>
          ))}
          {!data && (
