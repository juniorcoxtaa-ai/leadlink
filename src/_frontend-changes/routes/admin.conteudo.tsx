...
function AdminContent() {
  ...
  const [counts, setCounts] = useState({ links: 0, leads: 0 });

  useEffect(() => {
    (async () => {
      const [{ count: links }, { count: leads }] = await Promise.all([
        supabase.from("meu_link_configs").select("*", { count: "exact", head: true }),
        supabase.from("leads").select("*", { count: "exact", head: true }),
      ]);
      setCounts({ links: links || 0, leads: leads || 0 });
    })();
  }, []);
  ...
  return (
    <div className="space-y-5 max-w-[1300px]">
      ...
      <Card className="p-5">
        {section === "links" && <ContentTable title="Páginas públicas (Meu Link)" total={counts.links} />}
        {section === "quizzes" && <ContentTable title="Quizzes do funil" total={counts.links} />}
        ...
        {section === "imoveis" && <ContentTable title="Imóveis na plataforma" total={Math.floor(counts.links * 4.2)} />}
