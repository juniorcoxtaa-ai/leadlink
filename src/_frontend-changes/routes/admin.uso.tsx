...
function AdminUsage() {
  ...
  useEffect(() => {
    (async () => {
      const [{ data: leads }, { count: linksCount }, { count: profiles }] = await Promise.all([
        supabase.from("leads").select("slug,name,email"),
      ...
      (leads || []).forEach(l => {
        const key = l.slug || "—";
        const cur = grouped.get(key) || { count: 0, name: l.slug || "—", email: "" };
        cur.count++;
        grouped.set(key, cur);
      });
      ...
      setTotals({
        ...
        links: linksCount || 0,
        automations: Math.floor((profiles || 0) * 1.5),
        uploads: Math.floor((profiles || 0) * 4),
        visits: Math.floor((leads?.length || 0) * 7.3),
      });
    })();
  }, []);
