...
import { getBrokers } from "@/server-fns/leads";
...
type BrokerOption = Awaited<ReturnType<typeof getBrokers>>[number];
type LeadOption = Awaited<ReturnType<typeof getLeads>>[number];
type PropertyOption = Awaited<ReturnType<typeof getProperties>>[number];

interface Props {
  open: boolean;
  onOpenChange: (v: boolean) => void;
  initial?: Partial<Appointment> | null;
  onSave: (a: Appointment) => Promise<boolean | void>;
}
...
export function AppointmentFormDialog({ open, onOpenChange, initial, onSave }: Props) {
  ...
  useEffect(() => {
    getBrokers()
      .then((b) => {
        setBrokersList(b);
        setBrokerId((curr) => curr || b[0]?.id || "");
      })
      .catch(() => {});
    getLeads()
      .then((l) => setLeadsList(l.map((x: LeadOption) => ({ id: x.id, name: x.name }))))
      .catch(() => {});
    getProperties()
      .then((p) =>
        setPropertiesList(p.map((x: PropertyOption) => ({ id: x.id, title: x.title }))),
      )
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    ...
    setBrokerId(i.brokerId ?? brokersList[0]?.id ?? "");
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>Organize visitas, reuniões e ligações com seus leads.</SheetDescription>
        </SheetHeader>
        ...
        <div className="space-y-5 py-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              <Label className="flex items-center gap-1.5">
                <Building2 className="h-3.5 w-3.5" /> Imóvel (opcional)
              </Label>
              <Select
                value={propertyTitle || "_none"}
                onValueChange={(v) => setPropertyTitle(v === "_none" ? "" : v)}
              >
                <SelectTrigger>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Endereço, escritório, online..."
            />
          </div>
